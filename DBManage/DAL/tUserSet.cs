...
namespace LumluxSSYDB.DAL
{
	...
	public partial class tUserSet
	{
		...
		public bool Exists(string sGUID)
		{
			...
			strSql.Append("select count(1) from tUserSet");
		...
		/// 增加一条数据
		/// </summary>
		public bool Add(LumluxSSYDB.Model.tUserSet model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into tUserSet(");
			strSql.Append("sGUID,sKey,sValue,sUserGUID,sDesc,dCreateTime,dUpdateTime)");
			...
			SqlParameter[] parameters = {
					...
					new SqlParameter("@sUserGUID", SqlDbType.Char,36),
			...
			parameters[3].Value = model.sUserGUID;
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(LumluxSSYDB.Model.tUserSet model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update tUserSet set ");
			strSql.Append("sKey=@sKey,");
			strSql.Append("sValue=@sValue,");
			strSql.Append("sUserGUID=@sUserGUID,");
			...
			parameters[2].Value = model.sUserGUID;
		...
		public bool Delete(string sGUID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from tUserSet ");
			strSql.Append(" where sGUID=@sGUID ");
			SqlParameter[] parameters = {
					new SqlParameter("@sGUID", SqlDbType.Char,36)			};
			parameters[0].Value = sGUID;
		...
		/// 得到一个对象实体
		/// </summary>
		public LumluxSSYDB.Model.tUserSet GetModel(string sGUID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 sGUID,sKey,sValue,sUserGUID,sDesc,dCreateTime,dUpdateTime from tUserSet ");
			strSql.Append(" where sGUID=@sGUID ");
			SqlParameter[] parameters = {
					new SqlParameter("@sGUID", SqlDbType.Char,36)			};
			parameters[0].Value = sGUID;

			LumluxSSYDB.Model.tUserSet model=new LumluxSSYDB.Model.tUserSet();
		...
		/// 得到一个对象实体
		/// </summary>
		public LumluxSSYDB.Model.tUserSet DataRowToModel(DataRow row)
		{
			LumluxSSYDB.Model.tUserSet model=new LumluxSSYDB.Model.tUserSet();
			if (row != null)
			{
				...
				if(row["sUserGUID"]!=null)
				{
					model.sUserGUID=row["sUserGUID"].ToString();
				}
				if(row["sDesc"]!=null)
				{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select sGUID,sKey,sValue,sUserGUID,sDesc,dCreateTime,dUpdateTime ");
			strSql.Append(" FROM tUserSet ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from tUserSet T ");
